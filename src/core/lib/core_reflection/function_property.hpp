...
public:
	FunctionCollectionIteratorImpl(GetKeyFunc getKeyFunc, GetValueFunc getValueFunc, size_t index)
	    : getValueFunc_(getValueFunc), index_(index)
	{
	}
...
public:
	...
	template <typename TSizeFunc, typename TKeyFunc, typename TValueFunc, typename TAddKeyFunc>
	explicit FunctionCollection(TSizeFunc& getSizeFunc, TKeyFunc& getKeyFunc, TValueFunc& getValueFunc,
	                            TAddKeyFunc& addKeyFunc)
	    : getSizeFunc_(getSizeFunc), getKeyFunc_(getKeyFunc), getValueFunc_(getValueFunc), addKeyFunc_(addKeyFunc)
	{
	}
...
public:
	explicit FunctionCollectionProperty(const char* name, GetSizeFunc getSizeFunc, GetKeyFunc getKeyFunc,
	                                    GetValueFunc getValueFunc, AddKeyFunc addKeyFunc)
	    : BaseProperty(name, TypeId::getType<FunctionCollectionProperty>()), getSizeFunc_(getSizeFunc),
	      getValueFunc_(getValueFunc), getKeyFunc_(getKeyFunc), addKeyFunc_(addKeyFunc)
	{
	}
	...
	Variant get(const ObjectHandle& provider, const IDefinitionManager& definitionManager) const override
	{
		assert(this->isValue());
		auto pBase = reflectedCast<BaseType>(provider.data(), provider.type(), definitionManager);
		return Collection(std::make_shared<FunctionCollection<TKey, TValue>>(
		std::bind(getSizeFunc_, pBase),
		getKeyFunc_ ? std::bind(getKeyFunc_, pBase, std::placeholders::_1) :
		              FunctionCollection<TKey, TValue>::iterator_impl_type::GetKeyFunc(),
		std::bind(getValueFunc_, pBase, std::placeholders::_1), std::bind(addKeyFunc_, pBase, std::placeholders::_1)));
	}
