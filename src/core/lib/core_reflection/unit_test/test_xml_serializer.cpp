...
TEST_F( TestDefinitionFixture, serialize_simple_objects )
{
	...
	CHECK( is.good() );
	{
		...
		{
			const TestDefinitionDerivedObject * pObj1 = 
				dynamic_cast< TestDefinitionDerivedObject * >( objs_copy[i].get() );
			CHECK( pObj1 );

			const TestDefinitionDerivedObject * pObj2 = 
				dynamic_cast< TestDefinitionDerivedObject *>( objs[i].get() );
...
TEST_F( TestDefinitionFixture, serialize_properties )
{
	...
	props.push_back( 
		derived_klass_->bindProperty( "text", pObj.get() ) );
	props.push_back( 
		derived_klass_->bindProperty( "counter", pObj.get() ) );
