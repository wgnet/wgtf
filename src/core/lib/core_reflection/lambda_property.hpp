...
namespace wgt
{
class REFLECTION_DLL LambdaProperty :
public BaseProperty
{
	typedef BaseProperty base;

public:
	typedef std::function<Variant(
	const ObjectHandle& object,
	const IDefinitionManager& definitionManager)> Getter;

	typedef std::function<bool(
	const ObjectHandle& handle,
	const Variant& value,
	const IDefinitionManager& definitionManager)> Setter;
	...
	static IBasePropertyPtr create(
	const char* name,
	GetterT&& getter,
	MetaHandle metaData)
	{
		typedef typename std::decay<
		typename std::result_of<GetterT(ObjectType*)>::type
		>::type property_type;

		return std::make_shared<LambdaProperty>(
		name,
		TypeId::getType<property_type>(),
		wrapGetter<ObjectType>(std::forward<GetterT>(getter)),
		Setter(),
		std::move(metaData));
	}
	...
	private:
	    ...
	    static Getter wrapGetter(GetterT&& getter)
	    {
		    return [getter](
		           const ObjectHandle& object,
		           const IDefinitionManager& definitionManager) -> Variant
		    {
	    ...
	    struct NonNullSetterWrapper<SetterType, ObjectType, PropertyType, false>
	    {
		    ...
		    static Setter wrap(T&& setter)
		    {
			           ...
			           const IDefinitionManager& definitionManager) -> bool
			    {
					...
					bool result = false;
					value.visit<PropertyType>([&](const PropertyType& v)
					{
						result = setter(pObject, v);
					});
	    ...
	    struct NonNullSetterWrapper<SetterType, ObjectType, PropertyType, true>
	    {
		    ...
		    static Setter wrap(T&& setter)
		    {
			           ...
			           const IDefinitionManager& definitionManager) -> bool
			    {
					...
					return value.visit<PropertyType>([&](const PropertyType& v)
					{
						setter(pObject, v);
					});
				};
	    ...
	    public NonNullSetterWrapper<SetterType, ObjectType, PropertyType>
	    {
	    };
