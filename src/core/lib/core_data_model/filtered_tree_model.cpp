...
struct FilteredTreeModel::Implementation
{
	...
	std::vector<size_t>* findItemsToInsert(const IItem* parent, size_t index, size_t count, size_t& mappedIndex,
	                                       std::vector<size_t>& newIndices, std::vector<bool>& inFilter);
	std::vector<size_t>* findItemsToRemove(const IItem* parent, size_t index, size_t count, size_t& mappedIndex,
	                                       size_t& mappedCount);

	void shiftItems(size_t index, size_t offset, const IItem* parent, std::vector<size_t>& mappedIndices);
	void insertItems(size_t index, size_t offset, const IItem* parent, std::vector<size_t>& mappedIndices,
	                 std::vector<size_t>& newIndices, std::vector<bool>& inFilter, bool shift = true);
	void removeItems(size_t index, size_t count, size_t offset, const IItem* parent, std::vector<size_t>& mappedIndices,
	                 bool removeParent = true);

	FilterUpdateType checkUpdateType(const IItem* item, ItemIndex& sourceIndex, size_t& newIndex) const;
	void updateItem(const IItem* item, const ItemIndex& sourceIndex, size_t newIndex, FilterUpdateType type);
	...
	struct UpdateData
	{
		UpdateData() : parent_(nullptr), index_(0), count_(0), valid_(false)
		{
		}
...
FilteredTreeModel::Implementation::Implementation(FilteredTreeModel& self)
    : self_(self), model_(nullptr), filter_(nullptr)
{
	mapIndices();
	initialize();
}

FilteredTreeModel::Implementation::Implementation(FilteredTreeModel& self, const FilteredTreeModel::Implementation& rhs)
    : self_(self), model_(rhs.model_), filter_(rhs.filter_)
{
...
void FilteredTreeModel::Implementation::setSource(ITreeModel* source)
{
	...
	if (model_ != nullptr)
	{
		...
		std::bind(&FilteredTreeModel::Implementation::preItemDataChanged, this, _1, _2, _3, _4));
		connections_ += model_->signalPostItemDataChanged.connect(
		std::bind(&FilteredTreeModel::Implementation::postItemDataChanged, this, _1, _2, _3, _4));
		connections_ += model_->signalPreItemsInserted.connect(
		std::bind(&FilteredTreeModel::Implementation::preItemsInserted, this, _1, _2, _3));
		connections_ += model_->signalPostItemsInserted.connect(
		std::bind(&FilteredTreeModel::Implementation::postItemsInserted, this, _1, _2, _3));
		connections_ += model_->signalPreItemsRemoved.connect(
		std::bind(&FilteredTreeModel::Implementation::preItemsRemoved, this, _1, _2, _3));
		connections_ += model_->signalPostItemsRemoved.connect(
		std::bind(&FilteredTreeModel::Implementation::postItemsRemoved, this, _1, _2, _3));
		connections_ +=
		model_->signalDestructing.connect(std::bind(&FilteredTreeModel::Implementation::onDestructing, this));
	}
}
...
bool FilteredTreeModel::Implementation::empty(const IItem* item) const
{
	...
	for (size_t index = 0; index < max; ++index)
	{
		...
		if (this->filterMatched(child) || this->descendantFilterMatched(child))
		{
			return false;
		}
	}
...
std::vector<size_t>* FilteredTreeModel::Implementation::findItemsToInsert(const IItem* parent, size_t index,
                                                                          size_t count, size_t& mappedIndex,
                                                                          ...
                                                                          std::vector<bool>& inFilter)
{
	...
	bool ancestorInFilter = filterMatched(parent) || ancestorFilterMatched(parent);
	bool includeDueToAncestor = ancestorInFilter && !filter_->filterDescendantsOfMatchingItems();
...
std::vector<size_t>* FilteredTreeModel::Implementation::findItemsToRemove(const IItem* parent, size_t index,
                                                                          size_t count, size_t& mappedIndex,
                                                                          size_t& mappedCount)
{
	mappedCount = 0;
...
void FilteredTreeModel::Implementation::shiftItems(size_t index, size_t offset, const IItem* parent,
...
void FilteredTreeModel::Implementation::insertItems(size_t index, size_t offset, const IItem* parent,
                                                    std::vector<size_t>& mappedIndices, std::vector<size_t>& newIndices,
...
void FilteredTreeModel::Implementation::removeItems(size_t index, size_t count, size_t offset, const IItem* parent,
...
const IItem* item, ItemIndex& itemIndex, size_t& mappedIndex) const
{
	...
	bool includeDueToAncestor = ancestorFilterMatched(item) && !filter_->filterDescendantsOfMatchingItems();
	bool wasFilteredOut;
	bool nowFilteredOut = !includeDueToAncestor && !filterMatched(item) && !descendantFilterMatched(item);
...
void FilteredTreeModel::Implementation::updateItem(const IItem* item, const ItemIndex& itemIndex, size_t mappedIndex,
                                                   FilterUpdateType type)
{
	...
	case FilterUpdateType::REMOVE:
	{
		...
		{
			auto itr =
			std::lower_bound(mappedIndicesPointer->begin(), mappedIndicesPointer->end(), removePointIndex.first);
...
ITreeModel::ItemIndex FilteredTreeModel::Implementation::findInsertPoint(const IItem* parent, size_t index) const
{
	ItemIndex itemIndex;
...
ITreeModel::ItemIndex FilteredTreeModel::Implementation::findRemovePoint(const IItem* parent, size_t index,
                                                                         size_t count) const
{
	ItemIndex itemIndex;
	size_t max = self_.size(parent);
...
size_t FilteredTreeModel::Implementation::getSourceIndex(const IItem* parent, size_t index) const
{
...
bool FilteredTreeModel::Implementation::mapIndices(const IItem* parent, bool parentInFilter)
{
	...
	(ancestorFilterMatched(parent) || filterMatched(parent)) && !filter_->filterDescendantsOfMatchingItems();
	...
	for (size_t i = 0; i < max; ++i)
	{
		...
		if (item != nullptr)
		{
			if (includeDueToAncestor || filterMatched(item) || descendantFilterMatched(item))
			{
				indexFound = true;
				newIndices.push_back(i);
			}
...
void FilteredTreeModel::Implementation::remapIndices(const IItem* parent, bool parentInFilter)
{
	...
	size_t index = 0;
	bool includeDueToAncestor = parentInFilter && !filter_->filterDescendantsOfMatchingItems();
	...
	for (size_t i = 0; i < modelCount; ++i)
	{
		...
		bool itemInFilter = item == nullptr ? false : includeDueToAncestor || filterMatched(item);
...
void FilteredTreeModel::Implementation::preItemDataChanged(const IItem* item, int column, ItemRole::Id roleId,
                                                           const Variant& data)
{
...
void FilteredTreeModel::Implementation::postItemDataChanged(const IItem* item, int column, ItemRole::Id roleId,
                                                            const Variant& data)
{
	indexMapMutex_.unlock();
	...
	if (updateType == FilterUpdateType::UPDATE)
	{
		if (item != nullptr)
		{
			bool parentInFilter = ancestorFilterMatched(item) || filterMatched(item);
...
void FilteredTreeModel::Implementation::preItemsRemoved(const IItem* parent, size_t index, size_t count)
{
	...
	std::vector<size_t>* mappedIndicesPointer = findItemsToRemove(parent, index, sourceCount, mappedIndex, mappedCount);
...
void FilteredTreeModel::Implementation::postItemsRemoved(const IItem* parent, size_t index, size_t count)
{
	...
	if (lastUpdateData_.valid_)
	{
		...
		{
			self_.signalPostItemsRemoved(lastUpdateData_.parent_, lastUpdateData_.index_, lastUpdateData_.count_);
		}
...
FilteredTreeModel::FilteredTreeModel() : impl_(new Implementation(*this))
{
}

FilteredTreeModel::FilteredTreeModel(const FilteredTreeModel& rhs) : impl_(new Implementation(*this, *rhs.impl_))
{
}
...
IItem* FilteredTreeModel::item(size_t index, const IItem* parent) const
{
	...
	return sourceIndex == Implementation::INVALID_INDEX ? nullptr : impl_->model_->item(sourceIndex, parent);
}
...
size_t FilteredTreeModel::size(const IItem* item) const
{
	...
	if (childIndices == nullptr)
	{
		bool parentInFilter = impl_->ancestorFilterMatched(item) || impl_->filterMatched(item);
