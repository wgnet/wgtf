...
{
	MaxMinRangePair(const Variant& min, const Variant& max) : minValue_(min), maxValue_(max)
	{
	}
                                               ...
                                               IDefinitionManager& defMng) const
{
	...
	{
		auto descObj = findFirstMetaData<MetaDescriptionObj>(*item->getProperty(), defMng);
		if (descObj != nullptr)
		{
...
Variant UrlGetterExtension::getValue(const RefPropertyItem* item, int column, ItemRole::Id roleId,
                                     IDefinitionManager& defMng) const
{
...
void DefaultChildCreatorExtension::exposeChildren(const std::shared_ptr<const PropertyNode>& node,
	...
	if (node->propertyType == PropertyNode::RealProperty || node->propertyType == PropertyNode::CollectionItem)
	{
		...
		else if (propertyValue.tryCast(collectionHandle))
		{
			for (Collection::Iterator iter = collectionHandle.begin(); iter != collectionHandle.end(); ++iter)
			{
				...
				allocator->getCollectionItemProperty(std::move(name), std::move(valueType), defMng);
...
public:
	DefaultAllocator();
	~DefaultAllocator();
	std::shared_ptr<const PropertyNode> createPropertyNode(IBasePropertyPtr propertyInstance, ObjectHandle object,
	                                                       int32_t type = PropertyNode::RealProperty) override;
	IBasePropertyPtr getCollectionItemProperty(std::string&& name, const TypeId& type,
	                                           IDefinitionManager& defMng) override;

private:
	struct EmptyDeleter : public std::unary_function<ReflectedIteratorProperty*, void>
	{
	...
	struct PropertyHash : public std::unary_function<const std::shared_ptr<ReflectedIteratorProperty>, uint64_t>
	{
	...
	struct PropertyEqual : public std::binary_function<const std::shared_ptr<ReflectedIteratorProperty>,
	                                                   const std::shared_ptr<ReflectedIteratorProperty>, bool>
	{
		bool operator()(const std::shared_ptr<ReflectedIteratorProperty>& v1,
		                const std::shared_ptr<ReflectedIteratorProperty>& v2) const
		{
			return strcmp(v1->getName(), v2->getName()) == 0 && v1->getType() == v2->getType();
		}
	};
...
DefaultAllocator::DefaultAllocator() : pool(10000, 10)
{
}
...
std::shared_ptr<const PropertyNode> DefaultAllocator::createPropertyNode(IBasePropertyPtr propertyInstance,
                                                                         ObjectHandle object,
                                                                         int32_t type /*= PropertyNode::RealProperty*/)
{
...
RefPropertyItem* DefaultMergeValueExtension::lookUpItem(const std::shared_ptr<const PropertyNode>& node,
