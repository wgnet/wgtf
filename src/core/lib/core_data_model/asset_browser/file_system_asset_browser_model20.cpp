...
#include "file_system_asset_browser_model20.hpp"

#include "core_data_model/asset_browser/asset_list_model20.hpp"
#include "core_data_model/asset_browser/base_asset_object_item20.hpp"
#include "core_data_model/asset_browser/folder_tree_model20.hpp"
...
struct FileSystemAssetBrowserModel::FileSystemAssetBrowserModelImplementation
{
	...
	std::shared_ptr<AssetListModel> folderContents_;
	VariantList customContentFilters_;
	std::shared_ptr<AbstractTreeModel> folders_;
                                                          ...
                                                          definitionManager, presentationProvider))
{
	...
	impl_->folderContents_.reset(new AssetListModel);
...
void FileSystemAssetBrowserModel::populateFolderContents(AbstractItem* item)
{
...
AbstractListModel* FileSystemAssetBrowserModel::getFolderContents() const
{
	return impl_->folderContents_.get();
}

AbstractTreeModel* FileSystemAssetBrowserModel::getFolderTreeModel() const
{
...
IAssetObjectItem* FileSystemAssetBrowserModel::getAssetAtPath(const char* path, IAssetObjectItem* parent) const
{
	...
	int count = assetTree->rowCount(parent);
	for (int i = 0; i < count; ++i)
	{
		treeItem = dynamic_cast<IAssetObjectItem*>(assetTree->item(AbstractTreeModel::ItemIndex(i, parent)));
