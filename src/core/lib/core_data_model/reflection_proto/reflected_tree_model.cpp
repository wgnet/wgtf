...
{
	public:
	    ReflectedTreeModelPropertyListener(ReflectedTreeModel& model)
	        : model_(model)
	    {
		}
	    ...
	    virtual void preSetValue(const PropertyAccessor& accessor, const Variant& value) override
	    {
		    ...
		    typeId.isPointer() &&
		    definitionManager->getDefinition(typeId.removePointer().getName()) != nullptr;
	    ...
	    virtual void postInserted(const PropertyAccessor& accessor, size_t index, size_t count) override
	    {
		    ...
		    for (; i < index + count; ++i, ++collectionIt)
		    {
			    ...
			    auto property = new ReflectedPropertyItem(model_, itemObject, path);

			    propertyIt = properties->insert(propertyIt, std::unique_ptr<ReflectedPropertyItem>(property)) + 1;
			    ...
			    model_.mappedItems_.insert(std::make_pair(property, std::unique_ptr<ReflectedTreeModel::ItemMapping>(mapping)));
		    }
    ...
    MimeData ReflectedTreeModel::mimeData(std::vector<AbstractItemModel::ItemIndex>& indices)
    {
	    ...
	    mimeData[itemMimeKey] = std::vector<char>(data.begin(), data.end());
	    return mimeData;
    }
    ...
    std::vector<std::string> ReflectedTreeModel::mimeTypes() const
    {
	    ...
	    types.push_back(itemMimeKey);
	    return types;
    }

    bool ReflectedTreeModel::canDropMimeData(const MimeData& mimeData, DropAction action, const AbstractItemModel::ItemIndex& index) const
    {
	    auto it = mimeData.find(itemMimeKey);
	    if (it == mimeData.end())
	    {
		    return false;
	    }
	    ...
	    if (parentItem == nullptr || parentItem->getData(0, 0, ItemRole::isCollectionId) == false)
	    {
		    return false;
	    }
    ...
    bool ReflectedTreeModel::dropMimeData(const MimeData& mimeData, DropAction action, const AbstractItemModel::ItemIndex& index)
    {
	    auto it = mimeData.find(itemMimeKey);
	    if (it == mimeData.end())
	    {
    ...
    std::unique_ptr<ReflectedTreeModel::Children> ReflectedTreeModel::getChildren(const AbstractItem* item)
    {
	    ...
	    auto& properties = enumerateProperties(static_cast<const ReflectedPropertyItem*>(item));
    ...
    void ReflectedTreeModel::removeChildren(const AbstractItem* item, std::unique_ptr<Children>& children)
    {
	    clearProperties(static_cast<const ReflectedPropertyItem*>(item));
	    children.reset();
    }

    AbstractTreeModel::ItemIndex ReflectedTreeModel::childHint(const ReflectedPropertyItem* item)
    {
	    return ItemIndex(0, item);
    }

    const ReflectedTreeModel::Properties& ReflectedTreeModel::enumerateProperties(const ReflectedPropertyItem* item)
    {
			    ...
			    if (!value.tryCast(object))
			    {
				    ...
				    if (value.tryCast(collection))
				    {
					    ...
					    for (; it != collection.end(); ++it, ++i)
					    {
						    ...
						    std::string path = itemPath + "[" + std::to_string(static_cast<int>(indexKey)) + "]";
						    auto property = new ReflectedPropertyItem(*this, object, path);
	    ...
	    for (auto property : definition->allProperties())
	    {
		    ...
		    properties->emplace_back(new ReflectedPropertyItem(*this, object, path));
	    }
    ...
    void ReflectedTreeModel::unmapItem(const AbstractItem* item)
    {
	    ...
	    if (mapping->children_ != nullptr)
	    {
		    ...
		    removeChildren(item, mapping->children_);
	    }
	}
}
}