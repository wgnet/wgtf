...
public:
	...
	typedef void MoveSignature( const ItemIndex & sourceParentIndex,
		int sourceFirst,
		int sourceLast,
		const ItemIndex & destinationParentIndex,
	...
	virtual bool moveRows( const AbstractItem * sourceParent,
		int sourceRow,
		int count,
		const AbstractItem * destinationParent,
	...
	@param callback The callback to call.
	@return A Connection between the Signal and the callback. */
	virtual Connection connectPreRowsMoved( MoveCallback callback ) { return Connection(); }

	/** Adds a callback to call after rows are moved.
	@param callback The callback to call.
	@return A Connection between the Signal and the callback. */
	virtual Connection connectPostRowsMoved( MoveCallback callback ) { return Connection(); }
};
...
public:
	...
	@param sourceLast last row or column of removal under the parent.
	@param destinationFirst first row or column for insertion under the parent. */
	typedef void MoveSignature( int sourceFirst,
		int sourceLast,
		int destinationFirst );
	typedef std::function< MoveSignature > MoveCallback;
...
private:
	...
	bool moveRows( const AbstractItem * sourceParent,
		int sourceRow,
		int count,
		const AbstractItem * destinationParent,
	...
	{ 
		return connectPreItemDataChanged( ( DataCallback )[=](
			int row, int column, ItemRole::Id role, const Variant & value )
		{
	...
	{ 
		return connectPostItemDataChanged( ( DataCallback )[=](
			int row, int column, ItemRole::Id role, const Variant & value )
		{
			callback( AbstractItemModel::ItemIndex( row, column, nullptr ), role, value );
		}); 
	...
	{ 
		return connectPreRowsMoved( ( MoveCallback )[=]( int sourceFirst,
			int sourceLast,
			int destinationFirst )
		{
			callback( AbstractItemModel::ItemIndex(),
				sourceFirst,
				sourceLast,
				AbstractItemModel::ItemIndex(),
				destinationFirst );
		} ); 
	}
	...
	{ 
		return connectPostRowsMoved( ( MoveCallback )[=]( int sourceFirst,
			int sourceLast,
			int destinationFirst )
		{
			callback( AbstractItemModel::ItemIndex(),
...
public:
	...
	typedef void DataSignature(
		const ItemIndex & index, int column, ItemRole::Id role, const Variant & value );
	typedef std::function< DataSignature > DataCallback;
...
private:
	...
	Connection connectPreRowsMoved( AbstractItemModel::MoveCallback callback ) override
	{ 
		return connectPreRowsMoved( ( MoveCallback )[=]( const ItemIndex & sourceParent,
			int sourceFirst,
			int sourceLast,
			const ItemIndex & destinationParent,
			int destinationFirst )
		{
			callback(
				AbstractItemModel::ItemIndex( sourceParent.row_,
					0,
					sourceParent.parent_ ),
				sourceFirst,
				sourceLast,
				AbstractItemModel::ItemIndex( destinationParent.row_,
					0,
					destinationParent.parent_ ),
				destinationFirst );
		} ); 
	}
	...
	Connection connectPostRowsMoved( AbstractItemModel::MoveCallback callback ) override
	{ 
		return connectPostRowsMoved( ( MoveCallback )[=]( const ItemIndex & sourceParent,
			int sourceFirst,
			int sourceLast,
			const ItemIndex & destinationParent,
			int destinationFirst )
...
private:
	...
	{ 
		return connectPreItemDataChanged( ( DataCallback )[=](
			const ItemIndex & index, ItemRole::Id role, const Variant & value )
		{
