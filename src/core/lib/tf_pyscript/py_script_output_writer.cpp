...
static PyObject* write(PyObject* self, PyObject* args, bool isStderr)
{
	...
	{
		ScriptObject repr = ScriptObject(PyObject_Repr(PyTuple_GET_ITEM(args, 0)), ScriptObject::FROM_NEW_REFERENCE);

		handleWrite(std::string(PyString_AsString(repr.get()), PyString_GET_SIZE(repr.get())), isStderr);
...
PyMODINIT_FUNC PyInit_ScriptOutputWriter()
{
	...
	static PyMethodDef s_stdoutMethods[] = { { "write", &writeStdout, METH_VARARGS, "Write text to console" },
		                                     { "flush", &flush, METH_VARARGS, "Flush text to console" },
		                                     { NULL, NULL, 0, NULL } };

	static PyMethodDef s_stderrMethods[] = { { "write", &writeStderr, METH_VARARGS, "Write error to console" },
	...
	auto combinedModule = PyScript::ScriptModule::getOrCreate("scriptoutputwriter", PyScript::ScriptErrorPrint());
	...
	if (!buffer.empty() && buffer[buffer.size() - 1] == '\n')
	{
		...
		do
		{
			...
			std::string line(buffer.data() + lineStart,
			                 (lineEnd != std::string::npos) ? (lineEnd - lineStart) : (buffer.size() - lineStart));
			...
			lineStart = (lineEnd != std::string::npos) ? (lineEnd + 1) : std::string::npos;
