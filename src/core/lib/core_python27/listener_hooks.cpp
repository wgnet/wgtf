...
static PyObject* wrap_setattr(PyObject* self, PyObject* args, void* wrapped)
{
	...
	auto selfObject = PyScript::ScriptObject(self, PyScript::ScriptObject::FROM_BORROWED_REFERENCE);
	...
	auto tmpName = PyScript::ScriptObject(name, PyScript::ScriptObject::FROM_BORROWED_REFERENCE);
	auto nameObject = PyScript::ScriptString::create(tmpName);
	if (!nameObject.exists())
	{
		PyErr_Format(PyExc_AttributeError, "Cannot set attribute with no name.");
		return nullptr;
	}
	auto valueObject = PyScript::ScriptObject(value, PyScript::ScriptObject::FROM_BORROWED_REFERENCE);
	...
	auto rootHandle = ReflectedPython::DefinedInstance::find(context, rootInstance.pythonObject());
...
{
HookListener::HookListener() : entered_(0)
{
}

void HookListener::preSetValue(const PropertyAccessor& accessor, const Variant& value) /*override*/
{
	++entered_;
}

void HookListener::postSetValue(const PropertyAccessor& accessor, const Variant& value) /*override*/
{
	--entered_;
}
...
void getType(PyScript::ScriptObject& pythonObject, PyScript::ScriptClass& outClassObject,
             PyScript::ScriptType& outTypeObject, PyScript::ScriptDict& outTypeDict)
{
...
void attachListenerHooks(PyScript::ScriptObject& pythonObject)
{
	...
	{
		g_pyHookCountName =
		PyScript::ScriptString(PyString_InternFromString(g_hookCountName), PyScript::ScriptObject::FROM_NEW_REFERENCE);
	...
	auto hookCountObject = dict.getItem(g_pyHookCountName, PyScript::ScriptErrorClear());
	if (hookCountObject.exists())
	{
		...
		if (hookCountLong.exists())
		{
			...
			const auto setSuccess = dict.setItem(g_pyHookCountName, hookCountLong, PyScript::ScriptErrorClear());
			assert(setSuccess);
	...
	const auto setSuccess1 = dict.setItem(g_pyHookCountName, hookCountLong, PyScript::ScriptErrorClear());
	assert(setSuccess1);
	...
	auto setattrWrapperName =
	PyScript::ScriptString(setattrWrapperBase.name_strobj, PyScript::ScriptObject::FROM_BORROWED_REFERENCE);

	// Attach setattr hook
	auto originalSetattr = dict.getItem(setattrWrapperName, PyScript::ScriptErrorClear());
	if (originalSetattr != nullptr)
	{
		assert(g_pyOriginalSetattrName.exists());
		const auto setSuccess2 = dict.setItem(g_pyOriginalSetattrName, originalSetattr, PyScript::ScriptErrorClear());
		assert(setSuccess2);
	}
	...
	if (typeObject.exists())
	{
		...
		auto setattrWrapperObject =
		PyScript::ScriptDescrWrapper::create(typeObject, setattrWrapperBase, pFunctionToBeWrapped);
		assert(setattrWrapperObject.exists());
		const auto setSuccess3 =
		dict.setItem(setattrWrapperObject.name(), setattrWrapperObject, PyScript::ScriptErrorClear());
		assert(setSuccess3);

		// delattr
		assert(setattrWrapperBase.function == delattrWrapperBase.function);
		auto delattrWrapperObject =
		PyScript::ScriptDescrWrapper::create(typeObject, delattrWrapperBase, pFunctionToBeWrapped);
		assert(delattrWrapperObject.exists());
		const auto setSuccess4 =
		dict.setItem(delattrWrapperObject.name(), delattrWrapperObject, PyScript::ScriptErrorClear());
		assert(setSuccess4);
...
void detachListenerHooks(PyScript::ScriptObject& pythonObject)
{
	...
	auto delattrWrapperName =
	PyScript::ScriptString(delattrWrapperBase.name_strobj, PyScript::ScriptObject::FROM_BORROWED_REFERENCE);

	assert(g_pyOriginalSetattrName.exists());
	const auto originalSetattr = dict.getItem(g_pyOriginalSetattrName, PyScript::ScriptErrorClear());

	const auto setattrWrapperObject = dict.getItem(setattrWrapperName, PyScript::ScriptErrorClear());

	const auto delattrWrapperObject = dict.getItem(delattrWrapperName, PyScript::ScriptErrorClear());

	if (originalSetattr.exists())
	{
		const auto setSuccess1 = dict.setItem(setattrWrapperName, originalSetattr, PyScript::ScriptErrorClear());
		assert(setSuccess1);

		const auto setSuccess2 = dict.setItem(delattrWrapperName, originalSetattr, PyScript::ScriptErrorClear());
		assert(setSuccess2);

		const auto removeSuccess = dict.delItem(g_pyOriginalSetattrName, PyScript::ScriptErrorClear());
		assert(removeSuccess);
	}
	else
	{
		const auto removeSuccess1 = dict.delItem(setattrWrapperName, PyScript::ScriptErrorClear());
		assert(removeSuccess1);

		const auto removeSuccess2 = dict.delItem(delattrWrapperName, PyScript::ScriptErrorClear());
		assert(removeSuccess2);
	}

	assert(g_pyHookCountName.exists());
	const auto removeSuccess = dict.delItem(g_pyHookCountName, PyScript::ScriptErrorClear());
	assert(removeSuccess);
