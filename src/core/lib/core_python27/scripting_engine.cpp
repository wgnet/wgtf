...
ObjectHandle import(const char* name, IPythonObjManager& objManager)
{
	...
	PyScript::ScriptModule module =
	PyScript::ScriptModule::import(name, PyScript::ScriptErrorPrint("Unable to import\n"));
...
void initDebugger(const ICommandLineParser* commandLine)
{
	...
	auto pyDevModule = PyScript::ScriptModule::import("pydevd", PyScript::ScriptErrorRetain());
	...
	auto args = PyScript::ScriptArgs::create(host, stdoutToServer, stderrToServer, port, suspend);
	pyDevModule.callMethod("settrace", args, PyScript::ScriptErrorPrint());
}
...
bool Python27ScriptingEngine::init()
{
	...
	// Must be before Py_Initialize()
	PyImport_AppendInittab("scriptoutputwriter", PyScript::PyInit_ScriptOutputWriter);

	Py_Initialize();

	// Import the logging module
	// Must be after Py_Initialize()
	auto outputModule = PyScript::ScriptModule::import("scriptoutputwriter", PyScript::ScriptErrorPrint());
	...
	const auto result =
	swprintf(pythonSourcePath, BUFFER_SIZE, L"../../../src/core/third_party/python/python-%d.%d.%d/Lib",
