...
PyScript::ScriptCode getCodeFromData(
...
PyObject * WGTPythonImporter_find_module(PyObject *obj, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "s|O:WGTPythonImporter.find_module",
		&fullname, &path))
		return NULL;
...
bool setModuleProperties(
WGTPythonImporter* self,
const ModuleInfo& info,
PyScript::ScriptModule& mod)
{
	...
	{
		PyScript::ScriptString fullpath(PyString_FromFormat("%s%c%s",
		                                                    self->_path,
		                                                    SEP,
...
PyObject * WGTPythonImporter_load_module(PyObject *obj, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "s:WGTPythonImporter.load_module",
		&fullname))
	{
		return nullptr;
	}
	...
	if (isFileTypeSupported(info.type_))
	{
		...
		mod = PyScript::ScriptModule(
		PyImport_ExecCodeModuleEx(fullname, code.get(), (char*)info.fullPath.c_str()),
		PyScript::ScriptObject::FROM_NEW_REFERENCE);
		return mod.newRef();
	}
...
PyMethodDef WGTPythonImporter_methods[] = {
	{"find_module", WGTPythonImporter_find_module, METH_VARARGS,
	NULL},
	{"load_module", WGTPythonImporter_load_module, METH_VARARGS,
	NULL},
	{"iter_modules", WGTPythonImporter_iter_modules, METH_VARARGS,
	NULL},
	{"get_data", WGTPythonImporter_get_data, METH_VARARGS,
	NULL},
	{"get_code", WGTPythonImporter_get_code, METH_VARARGS,
	NULL},
	{"get_source", WGTPythonImporter_get_source, METH_VARARGS,
	NULL},
	{NULL,    NULL, 0 , NULL}
};
...
PyTypeObject WGTPythonImporter_Type = {
	...
	"WGTPythonImport.WGTPythonImporter",
...
void WGTPythonImport::init()
{
	...
	mod = Py_InitModule4("WGTPythonImport", NULL, NULL,
		NULL, PYTHON_API_VERSION);

	PythonImport_Detail::WGTPythonImportError =
	PyErr_NewException(const_cast<char*>("WGTPythonImport.WGTPythonImportError"),
	...
	if (PyModule_AddObject(mod, "WGTPythonImportError",
	                       PythonImport_Detail::WGTPythonImportError) < 0)
		return;
	// import WGTPythonImporter
	Py_INCREF(&PythonImport_Detail::WGTPythonImporter_Type);
	if (PyModule_AddObject(mod, "WGTPythonImporter",
	                       (PyObject*)&PythonImport_Detail::WGTPythonImporter_Type) < 0)
		return;
...
filetype type)
{
	...
	if (compileType == PY_SOURCE)
	{
		...
		return PyScript::ScriptCode(
		Py_CompileString(nullTerminatedData.c_str(), path.c_str(), Py_file_input),
		PyScript::ScriptObject::FROM_NEW_REFERENCE);
	}
...
PyScript::ScriptCode getModuleCode(const ModuleInfo& info)
{
	...
	return getCodeFromData(info.fullPath,
	                       blocks->cdata(),
	                       static_cast<int>(blocks->length()),
	                       info.type_);
}
