...
namespace
{
	...
	DefaultMetaTypeManager& s_metaTypeManager()
	{
		static DefaultMetaTypeManager s_instance;
		return s_instance;
	}
...
{
	auto type = findType< void >();
	if(!type)
	{
		typeInitError();
	}

	setTypeInternal( type->qualified( 0 ), Inline );
...
{
	value.type()->streamOut(stream, value.value< const void* >());
	return stream;
}
