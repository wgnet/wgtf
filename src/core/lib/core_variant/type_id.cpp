...
TypeId::TypeId(const char* name)
    : name_(name), hashCode_(name != nullptr ? HashUtilities::compute(name) : 0), owns_(false)
{
}

//==============================================================================
TypeId::TypeId(const char* name, uint64_t hashCode) : name_(name), hashCode_(hashCode), owns_(false)
{
}

//==============================================================================
TypeId::TypeId(const TypeId& other) : name_(other.name_), hashCode_(other.hashCode_), owns_(other.owns_)
{
	if (!owns_)
	{
		return;
...
{
	// Temporary hack - Using string manipulation to determine if a typeId points to a pointer type (or ObjectHandle
	// type)
