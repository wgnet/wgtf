...
public:
	...
	*/
	IProgressDialogPtr createProgressDialog(
	...
	*/
	IProgressDialogPtr createProgressDialog(
		const std::string& title, const std::string& label, uint32_t maximum)
	{
	...
	*/
	IProgressDialogPtr createProgressDialog(
		const std::string& title, const std::string& label, const std::string& cancelText, uint32_t maximum)
	{
	...
	*/
	virtual IProgressDialogPtr createProgressDialog(
		const std::string& title, const std::string& label, const std::string& cancelText,
		uint32_t minimum, uint32_t maximum, std::chrono::milliseconds duration) = 0;
	...
	*/
	IProgressDialog* createModelessProgressDialog(
		const std::string& title, const std::string& label, ModelessProgressCallback callback)
	{
		return createModelessProgressDialog(title, label, DefaultProgressCancelText, DefaultProgressMinimum,
			DefaultProgressMaximum, std::chrono::milliseconds(DefaultProgressDuration), callback);
	}
	...
	*/
	...
	const std::string& title, const std::string& label) = 0;
	...
	*/
	IProgressDialog* createModelessProgressDialog(
		const std::string& title, const std::string& label, uint32_t maximum, ModelessProgressCallback callback)
	{
	...
	*/
	IProgressDialog* createModelessProgressDialog(
		const std::string& title, const std::string& label, const std::string& cancelText,
		uint32_t minimum, uint32_t maximum, ModelessProgressCallback callback)
	{
	...
	*/
	virtual IProgressDialog* createModelessProgressDialog(
		...
		uint32_t maximum, std::chrono::milliseconds duration, ModelessProgressCallback callback) = 0;

	/*!
	Shows an open file dialog with default options and returns list of files. Caption for the dialog, working directory and file 
	...
	*/
	virtual const std::string showSelectDirectoryDialog(const std::string& caption,
	                                                    const std::string& directory, const QtFileDialogOptions& options) = 0;
	...
	virtual IComponent* findComponent(const TypeId& typeId,
	                                  std::function<bool(const ItemRole::Id&)>& predicate, const char* version) const = 0;
	...
	virtual std::unique_ptr<IDialog> createDialog(const char* resource,
	                                              ObjectHandleT<DialogModel> = nullptr) = 0;
