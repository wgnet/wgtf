...
class ActionManager	: public Depends< IUIFramework, IDefinitionManager >
{
public:
...
private:
	virtual std::unique_ptr<IAction> createAction(
		const char* text, const char* icon, const char* windowId, 
		const char* path, const char* shortcut, int order,
		std::function<void( IAction* )> func,
		std::function<bool( const IAction* )> enableFunc, 
		std::function<bool( const IAction* )> checkedFunc ) = 0;
