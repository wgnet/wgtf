...
namespace wgt
{
class SERIALIZATION_DLL TextStream : public BasicStream
{
	typedef BasicStream base;

public:
	explicit TextStream(IDataStream& dataStream) : base(dataStream)
	{
	}
...
private:
	...
	template <typename U>
	static Yes checkStreamingOut(
	typename std::remove_reference<decltype(std::declval<std::ostream&>() << std::declval<const U&>())>::type*);

	template <typename U>
	static No checkStreamingOut(...);

	template <typename U>
	static Yes checkStreamingIn(
	typename std::remove_reference<decltype(std::declval<std::istream&>() >> std::declval<U&>())>::type*);
...
template <typename T>
typename std::enable_if<std::is_pointer<T>::value, TextStream&>::type operator>>(TextStream& stream, T& v)
{
	void* tmp = (void*)v;
	stream >> tmp;
	v = (T)tmp;
