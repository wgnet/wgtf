...
FixedMemoryStream::FixedMemoryStream(void* buffer, std::streamsize size)
    : buffer_(static_cast<char*>(buffer)), readOnly_(false), pos_(0), size_(size)
{
}

FixedMemoryStream::FixedMemoryStream(const void* buffer, std::streamsize size)
    : buffer_(static_cast<char*>(const_cast<void*>(buffer))), readOnly_(true), pos_(0), size_(size)
{
}

FixedMemoryStream::FixedMemoryStream(const char* buffer)
    : buffer_(const_cast<char*>(buffer)), readOnly_(true), pos_(0), size_(buffer ? std::strlen(buffer) : 0)
{
}
