 ...
 */
int setData(PyObject* pObject, bool& rBool, const char* varName)
{
 ...
 *	@return 0 for success, -1 for error (like pySetAttribute)
 */
int setData(PyObject* pObject, int& rInt, const char* varName)
{
	if (PyInt_Check(pObject))
	{
 ...
 *	@return 0 for success, -1 for error (like pySetAttribute)
 */
int setData(PyObject* pObject, long& rInt, const char* varName)
{
 ...
 *	@return 0 for success, -1 for error (like pySetAttribute)
 */
int setData(PyObject* pObject, int64_t& rInt, const char* varName)
{
	if (PyLong_Check(pObject))
	{
 ...
 *	@return 0 for success, -1 for error (like pySetAttribute)
 */
int setData(PyObject* pObject, uint64_t& rUint, const char* varName)
{
	if (PyLong_Check(pObject))
	{
	...
	if (PyInt_Check(pObject))
	{
		...
		{
			PyErr_Format(PyExc_ValueError, "Cannot set %s of type unsigned long to %d", varName, int(intValue));
			return -1;
		}
	}
 ...
 *	@return 0 for success, -1 for error (like pySetAttribute)
 */
int setData(PyObject* pObject, unsigned int& rUint, const char* varName)
{
	if (PyInt_Check(pObject))
	{
	...
	if (PyLong_Check(pObject))
	{
		...
		if (!PyErr_Occurred() && (asLong >= 0) && (asLong == static_cast<long>(rUint)))
		{
			return 0;
		}
	}
 ...
 *	@return 0 for success, -1 for error (like pySetAttribute)
 */
int setData(PyObject* pObject, float& rFloat, const char* varName)
{
	double d;
 ...
 *	@return 0 for success, -1 for error (like pySetAttribute)
 */
int setData(PyObject* pObject, double& rDouble, const char* varName)
{
	if (PyFloat_Check(pObject))
	{
 ...
 *	@return 0 for success, -1 for error (like pySetAttribute)
 */
int setData(PyObject* pObject, PyObject*& rPyObject, const char* /*varName*/)
{
 ...
 */
int setData(PyObject* pObject, PyObjectPtr& rPyObject, const char* /*varName*/)
{
	PyObject* pSet = (pObject != Py_None) ? pObject : NULL;

	if (rPyObject.get() != pSet)
		rPyObject = pSet;
 ...
 *	@return 0 for success, -1 for error (like pySetAttribute)
 */
int setData(PyObject* pObject, std::string& rString, const char* varName)
{
	PyObjectPtr pUTF8String;

	if (PyUnicode_Check(pObject))
	{
		pUTF8String = PyObjectPtr(PyUnicode_AsUTF8String(pObject), true /*STEAL_REFERENCE*/);
 ...
 *	@return 0 for success, -1 for error (like pySetAttribute)
 */
int setData(PyObject* pObject, std::wstring& rString, const char* varName)
{
	if (PyString_Check(pObject) || PyUnicode_Check(pObject))
	{
		...
		if (pUO)
		{
			...
			if (ulen >= 0)
			{
				...
				Py_ssize_t nChars = PyUnicode_AsWideChar(unicodeObj, &rString[0], ulen);
...
PyObject* getData(const unsigned int data)
{
	...
	return (long(asULong) < 0) ? PyLong_FromUnsignedLong(asULong) : PyInt_FromLong(asULong);
}
...
{
	PyObject* ret = (data ? const_cast<PyObject*>(data.get()) : Py_None);
	Py_INCREF(ret);
	return ret;
}
...
{
	PyObject* pRet = PyString_FromStringAndSize(const_cast<char*>(data.data()), data.size());
...
{
	PyObject* pRet = PyUnicode_FromWideChar(const_cast<wchar_t*>(data.c_str()), data.size());
