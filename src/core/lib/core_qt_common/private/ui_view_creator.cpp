                                                            ...
                                                            const char* uniqueName)
{
	...
	auto view = uiFrameWork->createViewAsync(uniqueName, path, IUIFramework::ResourceType::Url, context,
...
//------------------------------------------------------------------------------
wg_future<std::unique_ptr<IView>> UIViewCreator::createView(const char* path, const Variant& context,
                                                            std::function<void(IView&)> functor, const char* uniqueName)
{
	auto uiFrameWork = get<IUIFramework>();
	if (uiFrameWork == nullptr)
	{
...
//------------------------------------------------------------------------------
void UIViewCreator::createWindow(const char* path, const Variant& context,
                                 ...
                                 IUIFramework::ResourceType type)
{
	...
	uiFrameWork->createWindowAsync(path, type, context, [uiApplication, functor](std::unique_ptr<IWindow>& view) {
		if (view)
		{
