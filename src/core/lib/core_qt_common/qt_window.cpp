...
struct QtWindow::Impl : Depends<IQtFramework>
{
	QtWindow& window_;
	...
	Impl(IComponentContext& context, QIODevice& source, QtWindow& window)
	    : Depends(context), window_(window), isMaximizedInPreference_(false), firstTimeShow_(true),
...
QtWindow::QtWindow(IComponentContext& context, QIODevice& source) : impl_(new Impl(context, source, *this))
{
	impl_->init();
}
