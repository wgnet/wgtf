...
public:
	bool toVariant(const QVariant& qVariant, Variant& o_variant) const override
	{
		if (qVariant.type() == QVariant::Vector2D)
		{
			...
			o_variant = Vector2(vectorData.x(), vectorData.y());
			return true;
		}
		...
		if (qVariant.type() == QVariant::Vector3D)
		{
			...
			o_variant = Vector3(vectorData.x(), vectorData.y(), vectorData.z());
			return true;
		}
		...
		if (qVariant.type() == QVariant::Vector4D)
		{
			...
			o_variant = Vector4(vectorData.x(), vectorData.y(), vectorData.z(), vectorData.w());
			return true;
		}
	...
	bool toQVariant(const Variant& variant, QVariant& o_qVariant, QObject* parent = nullptr) const override
	{
		if (variant.typeIs<Vector2>())
		{
			...
			o_qVariant = QVariant::fromValue(QVector2D(value.x, value.y));
			return true;
		}
		...
		if (variant.typeIs<Vector3>())
		{
			...
			o_qVariant = QVariant::fromValue(QVector3D(value.x, value.y, value.z));
			return true;
		}
		...
		if (variant.typeIs<Vector4>())
		{
			...
			o_qVariant = QVariant::fromValue(QVector4D(value.x, value.y, value.z, value.w));
			return true;
		}
