...
bool indertimanate)
{
	progressDialog_.reset(new QProgressDialog(QString::fromStdString(label),
	                                          cancelText.empty() ? QString() : QString::fromStdString(cancelText),
	                                          min, max, 0, 0));
	...
	if (indertimanate)
	{
		...
		progressBar_.reset(new QProgressBar(progressDialog_.get()));
		progressBar_->setFormat(" ");
		progressBar_->setMaximum(0);
		progressBar_->setMinimum(0);
		progressDialog_->setBar(progressBar_.get());
	...
	void QtProgressDialog::setCancelButtonText(const std::string& cancelButtonText)
	{
		if (progressDialog_)
		{
			...
			{
				cancelConnection_ = QObject::connect(progressDialog_.get(), &QProgressDialog::canceled, [this](){
					progressDialog_->reset();
				});
			}
			return progressDialog_->setCancelButtonText(cancelButtonText.empty() ? QString() : QString::fromStdString(cancelButtonText));
		}
	}
