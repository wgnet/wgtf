...
class ExtensionData : public IExtensionData
{
	typedef QMap<ItemRole::Id, QVariant> ItemData;
	    ...
	    public:
	        ExtensionData(QAbstractItemModel* extendedModel)
	        ...
	        void dataExtChanged(const QModelIndex& topLeft, const QModelIndex& bottomRight, const QVector<ItemRole::Id> roleIds) override
	        {
		        QVector<int> roles;
	    ...
	    public:
	        ExtendedModel(QList<IModelExtension*>& extensions)
	            : model_(nullptr)
	        ...
	        void reset(QAbstractItemModel* model)
	        {
		        ...
		        if (model_ != nullptr)
		        {
			        ...
			        connections_ += QObject::connect(model, &QAbstractItemModel::rowsAboutToBeMoved, this, &ExtendedModel::onRowsAboutToBeMoved);
			        connections_ += QObject::connect(model, &QAbstractItemModel::rowsMoved, this, &ExtendedModel::onRowsMoved);
			        connections_ += QObject::connect(model, &QAbstractItemModel::columnsAboutToBeInserted, this, &ExtendedModel::onColumnsAboutToBeInserted);
			        connections_ += QObject::connect(model, &QAbstractItemModel::columnsInserted, this, &ExtendedModel::onColumnsInserted);
			        connections_ += QObject::connect(model, &QAbstractItemModel::columnsAboutToBeRemoved, this, &ExtendedModel::onColumnsAboutToBeRemoved);
			        connections_ += QObject::connect(model, &QAbstractItemModel::columnsRemoved, this, &ExtendedModel::onColumnsRemoved);
			        connections_ += QObject::connect(model, &QAbstractItemModel::columnsAboutToBeMoved, this, &ExtendedModel::onColumnsAboutToBeMoved);
			        connections_ += QObject::connect(model, &QAbstractItemModel::columnsMoved, this, &ExtendedModel::onColumnsMoved);

			        for (auto& extension : extensions_)
			        {
				        connections_ += QObject::connect(this, &QAbstractItemModel::dataChanged, extension, &IModelExtension::onDataChanged);
				        ...
				        connections_ += QObject::connect(this, &QAbstractItemModel::rowsAboutToBeMoved, extension, &IModelExtension::onRowsAboutToBeMoved);
				        connections_ += QObject::connect(this, &QAbstractItemModel::rowsMoved, extension, &IModelExtension::onRowsMoved);
				        connections_ += QObject::connect(this, &QAbstractItemModel::columnsAboutToBeInserted, extension, &IModelExtension::onColumnsAboutToBeInserted);
				        connections_ += QObject::connect(this, &QAbstractItemModel::columnsInserted, extension, &IModelExtension::onColumnsInserted);
				        connections_ += QObject::connect(this, &QAbstractItemModel::columnsAboutToBeRemoved, extension, &IModelExtension::onColumnsAboutToBeRemoved);
				        connections_ += QObject::connect(this, &QAbstractItemModel::columnsRemoved, extension, &IModelExtension::onColumnsRemoved);
				        connections_ += QObject::connect(this, &QAbstractItemModel::columnsAboutToBeMoved, extension, &IModelExtension::onColumnsAboutToBeMoved);
				        connections_ += QObject::connect(this, &QAbstractItemModel::columnsMoved, extension, &IModelExtension::onColumnsMoved);
			        }
	        ...
	        QMimeData* mimeData(const QModelIndexList& indexes) const override
	        {
		        ...
		        extendedData->setData(itemMimeKey, encodedIndexes.c_str());
	        ...
	        bool canDropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex& parent) const override
	        {
		        ...
		        return data->hasFormat(itemMimeKey) && action == Qt::MoveAction;
	        }
	        ...
	        bool dropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex& parent) override
	        {
		        ...
		        bool canDrop = (data->hasFormat(itemMimeKey) && action == Qt::MoveAction);
		        if (!canDrop)
		        {
			        return false;
		        }
		        ...
		        std::string decoded;
		        Base64::decode(data->data(itemMimeKey).data(), decoded);
		        ...
		        QPersistentModelIndex persistentDestination = index(row > persistentIndexes[0].row() ? row + 1 : row, column, parent);
	    ...
	    private:
	        ...
	        bool setHeaderData(int section, Qt::Orientation orientation, const QVariant& value, int role) override
	        {
		        ...
		        return model_->setHeaderData(orientation == Qt::Horizontal ? sourceColumn(section) : section, orientation, value, role);
	        }
	        ...
	        void onRowsAboutToBeMoved(const QModelIndex& sourceParent,
	                                  ...
	                                  int destinationRow)
	        {
		        beginMoveRows(extendedIndex(sourceParent),
		                      sourceFirst,
		                      sourceLast,
		                      extendedIndex(destinationParent),
		                      destinationRow);
	        }
	        ...
	        void onColumnsAboutToBeRemoved(const QModelIndex& parent, int first, int last)
	        {
		        ...
		        for (int i = 0; i < columnMappings_.size(); ++i)
		        {
			        ...
			        {
				        beginRemoveColumns(QModelIndex(), first, last);
				        columnMappings_.erase(
				        columnMappings_.begin() + first,
				        columnMappings_.begin() + last + 1);
				        endRemoveColumns();
				        i -= last - first + 1;
				        first = -1;
				        last = -1;
	        ...
	        void onColumnsAboutToBeMoved(const QModelIndex& sourceParent,
	                                     int sourceFirst,
	                                     int sourceLast,
	                                     const QModelIndex& destinationParent,
	        ...
	        void onColumnsMoved(const QModelIndex& sourceParent,
	                            int sourceFirst,
	                            int sourceLast,
	                            const QModelIndex& destinationParent,
	                            int destinationColumn)
	        ...
	        QModelIndex sourceIndex(const QModelIndex& extendedIndex, int row = -1, int column = -1) const
	        {
		        ...
		        return createIndex(row == -1 ? extendedIndex.row() : row, column == -1 ? sourceColumn(extendedIndex.column()) : column, extendedIndex.internalPointer());
	        }
	        ...
	        QModelIndex extendedIndex(const QModelIndex& modelIndex, int row = -1, int column = -1) const
	        {
		        ...
		        return createIndex(row == -1 ? modelIndex.row() : row, column == -1 ? extendedColumn(modelIndex.column()) : column, modelIndex.internalId());
	        }
        ...
        : impl_(new Impl())
    {
	    ...
	    QObject::connect(impl_->extendedModel_.get(), &QAbstractItemModel::modelReset, [&]() { rowCountChanged(); columnCountChanged(); refreshHeaderData(); });
	    QObject::connect(impl_->extendedModel_.get(), &QAbstractItemModel::rowsInserted, [&]() { rowCountChanged(); refreshHeaderData(); });
	    QObject::connect(impl_->extendedModel_.get(), &QAbstractItemModel::rowsRemoved, [&]() { rowCountChanged(); refreshHeaderData(); });
	    QObject::connect(impl_->extendedModel_.get(), &QAbstractItemModel::rowsMoved, [&]() { refreshHeaderData(); });
	    QObject::connect(impl_->extendedModel_.get(), &QAbstractItemModel::columnsInserted, [&]() { columnCountChanged(); refreshHeaderData(); });
	    QObject::connect(impl_->extendedModel_.get(), &QAbstractItemModel::columnsRemoved, [&]() { columnCountChanged(); refreshHeaderData(); });
    ...
    void WGItemView::setModel(QAbstractItemModel* model)
    {
	    ...
	    {
		    impl_->connections_ += QObject::connect(impl_->model_, &QAbstractItemModel::modelReset, [&]() { impl_->extendedModel_->reset(impl_->model_); });
	    }
    }
    ...
    QStringList WGItemView::headerMimeTypes(Qt::Orientation orientation) const
    {
	    ...
	    {
		    mimeTypes.append(horizontalHeaderMimeKey);
	    }
	    else if (orientation == Qt::Vertical)
	    {
		    mimeTypes.append(verticalHeaderMimeKey);
	    }
	    return mimeTypes;
    }
    ...
    QVariantMap WGItemView::headerMimeData(const QList<int>& sections, Qt::Orientation orientation) const
    {
	    ...
	    QVariantMap data;
	    data.insert(orientation == Qt::Horizontal ?
	                horizontalHeaderMimeKey :
	                verticalHeaderMimeKey,
	                encodedSections.c_str());
	    return data;
    }

    bool WGItemView::canDropHeaderMimeData(const QVariantMap& data, Qt::DropAction action, int section, Qt::Orientation orientation) const
    {
	    return data.find(orientation == Qt::Horizontal ? horizontalHeaderMimeKey : verticalHeaderMimeKey) != data.end() && action == Qt::MoveAction;
    }

    bool WGItemView::dropHeaderMimeData(const QVariantMap& data, Qt::DropAction action, int section, Qt::Orientation orientation)
    {
	    ...
	    QList<int> sections;
	    std::string decoded;
	    auto it = data.find(orientation == Qt::Horizontal ? horizontalHeaderMimeKey : verticalHeaderMimeKey);
