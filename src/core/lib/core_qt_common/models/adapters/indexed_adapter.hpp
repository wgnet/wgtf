...
struct IndexedAdapter
{
	...
	IndexedAdapter(QPersistentModelIndex index, Adapter* data) : index_(index), data_(data)
	{
	}

	IndexedAdapter(IndexedAdapter<Adapter>&& rhs) : index_(rhs.index_), data_(rhs.data_.release())
	{
	}
...
template <typename T>
void isolateRedundantIndices(std::vector<IndexedAdapter<T>>& indices,
                             std::vector<std::unique_ptr<T>>& redundantAdapters)
{
	for (;;)
	{
		auto predicate = [](const IndexedAdapter<T>& item) { return !item.index_.isValid(); };
...
template <typename T>
void isolateRedundantIndices(const QModelIndex& parent, std::vector<IndexedAdapter<T>>& indices,
                             std::vector<std::unique_ptr<T>>& redundantAdapters)
{
	for (;;)
	{
...
template <typename T>
void isolateRedundantIndices(const QModelIndex& parent, int firstRow, int lastRow,
                             ...
                             std::vector<std::unique_ptr<T>>& redundantAdapters)
{
	for (auto row = firstRow; row <= lastRow; ++row)
	{
		auto predicate = [&parent, row](const IndexedAdapter<T>& item) {
			...
			return itemParent.internalId() == parent.internalId() && itemRow == row;
		};
		auto it = std::find_if(indices.begin(), indices.end(), predicate);
		isolateRedundantIndex(it, indices, redundantAdapters);
	}
...
template <typename T>
void isolateRedundantIndex(const typename std::vector<IndexedAdapter<T>>::iterator& it,
                           std::vector<IndexedAdapter<T>>& indices, std::vector<std::unique_ptr<T>>& redundantAdapters)
{
	if (it == indices.end())
	{
		return;
