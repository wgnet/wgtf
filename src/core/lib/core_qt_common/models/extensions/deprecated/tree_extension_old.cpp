...
void TreeExtensionOld::Implementation::collapse(const QModelIndex& index)
{
	...
	if (hasPath)
	{
		...
		{
			std::swap(expandedList_[it - expandedList_.begin()], expandedList_[expandedList_.size() - 1]);
	...
	else
	{
		...
		{
			std::swap(memoryExpandedList_[it - memoryExpandedList_.begin()],
...
bool TreeExtensionOld::Implementation::expanded(const QModelIndex& index) const
{
	...
	if (hasPath)
	{
		return std::find(expandedList_.cbegin(), expandedList_.cend(), indexPath) != expandedList_.cend();
	}
	else
	{
		auto item = reinterpret_cast<IItem*>(index.internalPointer());
...
void TreeExtensionOld::Implementation::saveStates(const char* id)
{
	...
	if (count == 0)
	{
		...
		{
			NGT_WARNING_MSG("Tree preference won't save for WGTreeModel: %s, %s\n", id,
...
QVariant TreeExtensionOld::data(const QModelIndex& index, int role) const
{
	...
	{
		if (!model->hasChildren(index) || !impl_->expanded(index))
		{
			return QVariant(QVariant::Invalid);
		}
...
void TreeExtensionOld::onLayoutAboutToBeChanged(const QList<QPersistentModelIndex>& parents,
...
void TreeExtensionOld::onLayoutChanged(const QList<QPersistentModelIndex>& parents,
                                       QAbstractItemModel::LayoutChangeHint hint)
{
...
void TreeExtensionOld::onRowsAboutToBeRemoved(const QModelIndex& parent, int first, int last)
{
	isolateRedundantIndices(parent, first, last, impl_->childModels_, impl_->redundantChildModels_);
}

void TreeExtensionOld::onRowsRemoved(const QModelIndex& parent, int first, int last)
{
	impl_->redundantChildModels_.clear();
}
