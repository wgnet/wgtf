...
QHash<int, QByteArray> ValueExtension::roleNames() const
{
	...
	registerRole(NameRole::roleName_, roleNames);
	registerRole(DefinitionModelRole::roleName_, roleNames);
	registerRole(DescriptionRole::roleName_, roleNames);
...
QVariant ValueExtension::data(const QModelIndex& index, int role) const
{
	...
	if (roleId == ValueRole::roleId_ || roleId == ValueTypeRole::roleId_ || roleId == NameRole::roleId_ ||
	    roleId == ObjectRole::roleId_ || roleId == RootObjectRole::roleId_ || roleId == IndexPathRole::roleId_ ||
	    roleId == KeyRole::roleId_ || roleId == KeyTypeRole::roleId_ || roleId == MinValueRole::roleId_ ||
	    roleId == MaxValueRole::roleId_ || roleId == StepSizeRole::roleId_ || roleId == DecimalsRole::roleId_ ||
	    roleId == EnumModelRole::roleId_ || roleId == DefinitionRole::roleId_ ||
	    roleId == DefinitionModelRole::roleId_ || roleId == UrlIsAssetBrowserRole::roleId_ ||
...
void ValueExtension::onDataAboutToBeChanged(const QModelIndex& index, int role, const QVariant& value)
{
	...
	if (roleId == DefinitionRole::roleId_)
	{
		...
		emit const_cast<QAbstractItemModel*>(model)->layoutAboutToBeChanged(parents,
...
{
	return QQmlListProperty<QString>(const_cast<ValueExtension*>(this), nullptr, &appendRole, &countRoles, &roleAt,
	                                 &clearRoles);
}

void ValueExtension::appendRole(QQmlListProperty<QString>* property, QString* value)
{
...
QString* ValueExtension::roleAt(QQmlListProperty<QString>* property, int index)
{
	auto valueExtension = qobject_cast<ValueExtension*>(property->object);
	if (valueExtension == nullptr)
	{
...
void ValueExtension::clearRoles(QQmlListProperty<QString>* property)
{
	auto valueExtension = qobject_cast<ValueExtension*>(property->object);
	if (valueExtension == nullptr)
	{
...
int ValueExtension::countRoles(QQmlListProperty<QString>* property)
{
	auto valueExtension = qobject_cast<ValueExtension*>(property->object);
	if (valueExtension == nullptr)
	{
