...
{
	template<typename T>
	QQmlPrivate::RegisterType TryRegisterType( const char *uri, int versionMajor, int versionMinor, const char *qmlName, int metaObjectRevision )
	{
		QML_GETTYPENAMES

		QQmlPrivate::RegisterType type = {
			metaObjectRevision == 0 ? 0 : 1,
	...
	template<typename T>
	void registerType( IUIFramework* framework, char *uri, int versionMajor, int versionMinor, const char *qmlName, int metaObjectRevision = 0 )
	{
