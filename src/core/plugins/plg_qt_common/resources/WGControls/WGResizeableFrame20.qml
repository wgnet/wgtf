...
Item {
    ...
    WGComponent { type: "WGResizeableFrame20" }
    ...
    */
    ...
    /*! Signal that the targetItem should set its dimensions to the new values given. */
    ...
    MouseArea {
        ...
        onPressed: {
            __initialX = x
            __initialY = y
            __initialWidth = targetItem.width
            __initialHeight = targetItem.height
            anchors.left = undefined
            anchors.top = undefined
        }

        onReleased: {
            setDimensions(targetItem.x + (tLeftCornerDrag.x - __initialX),
                targetItem.y + (tLeftCornerDrag.y - __initialY),
                __initialWidth - (tLeftCornerDrag.x - __initialX),
                __initialHeight - (tLeftCornerDrag.y - __initialY));
            anchors.left = parent.left
            anchors.top = parent.top
        }
    ...
    MouseArea {
        ...
        onReleased: {
            setDimensions(targetItem.x,
                targetItem.y + (tRightCornerDrag.y - __initialY),
                __initialWidth + (tRightCornerDrag.x - __initialX),
                __initialHeight - (tRightCornerDrag.y - __initialY));
            anchors.right = parent.right
            anchors.top = parent.top
    ...
    MouseArea {
        ...
        onReleased: {
            setDimensions(targetItem.x,
                targetItem.y,
                __initialWidth + (bRightCornerDrag.x - __initialX),
                __initialHeight + (bRightCornerDrag.y - __initialY));
            anchors.right = parent.right
            anchors.bottom = parent.bottom
    ...
    MouseArea {
        ...
        onReleased: {
            setDimensions(targetItem.x + (bLeftCornerDrag.x - __initialX),
                targetItem.y,
                __initialWidth - (bLeftCornerDrag.x - __initialX),
                __initialHeight + (bLeftCornerDrag.y - __initialY));
            anchors.left = parent.left
            anchors.bottom = parent.bottom
    ...
    MouseArea {
        ...
        onReleased: {
            setDimensions(targetItem.x,
                targetItem.y + (topDrag.y - __initialY),
                targetItem.width,
                __initialHeight - (topDrag.y - __initialY));
            anchors.top = parent.top
            groupBox.anchors.top = tLeftCornerDrag.top
        }
    ...
    MouseArea {
        ...
        onReleased: {
            setDimensions(targetItem.x,
                targetItem.y,
                __initialWidth + (rightDrag.x - __initialX),
                targetItem.height);
            anchors.right = parent.right
            groupBox.anchors.right = tRightCornerDrag.right
        }
    ...
    MouseArea {
        ...
        onReleased: {
            setDimensions(
                ...
                targetItem.y,
                targetItem.width,
                __initialHeight + (bottomDrag.y - __initialY));
            anchors.bottom = parent.bottom
            groupBox.anchors.bottom = bLeftCornerDrag.bottom
        }
    ...
    MouseArea {
        ...
        onReleased: {
            setDimensions(
                targetItem.x + (leftDrag.x - __initialX),
                targetItem.y,
                __initialWidth - (leftDrag.x - __initialX),
                targetItem.height);
            anchors.left = parent.left
            groupBox.anchors.left = tLeftCornerDrag.left
        }
