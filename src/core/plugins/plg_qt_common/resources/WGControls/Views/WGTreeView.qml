...
\endcode
  ...
  \todo - Test with various configurations of depthColourisation and flatColourisation
        - Requires extensive testing with indentation and leafNodeIndentation
*/

Item {
    id: treeView
    objectName: typeof(itemData) != "undefined" ? itemData.indexPath : "WGTreeView"
    WGComponent { type: "WGTreeView" }
    ...
    function calculateMaxTextWidthHelper(parentObject, currentDepth, column){
        ...
        for (var i=0; i<parentObject.children.length; i++)
        {
            ...
            if (childObject.visible)
            {
                ...
                if (typeof childObject.autoSizeOnDoubleClick != "undefined")
                {
                    if(typeof childObject.elide != "undefined")
                    {
                    ...
                    {
                        var paintedWidth = 0;
                        for (var j = 0; j < childObject.children.length; j++)
                        {
                            if(typeof childObject.children[j].contentWidth != "undefined")
                            {
                                paintedWidth = Math.max(paintedWidth, childObject.children[j].contentWidth);
                            }
                        }
                        maxTextWidth = Math.max(maxTextWidth, paintedWidth + defaultSpacing.standardMargin);
                    }
                }
