...
TestListModel::Implementation::Implementation(TestListModel& self, bool shortList) : self_(self), shortList_(shortList)
{
	generateData();
...
char* TestListModel::Implementation::copyString(const std::string& s) const
{
...
void TestListModel::Implementation::generateData()
{
	...
	while (!dataString.empty())
	{
		if (!shortList_ && items_.size() % 3 == 0)
		{
			...
			items_.push_back(new TestListItem(dataString.c_str(), colour));
		}
		else
		{
			items_.push_back(new TestListItem(dataString.c_str(), dataString.c_str()));
		}
...
TestListModel::TestListModel(bool shortList) : impl_(new Implementation(*this, shortList))
{
}

TestListModel::TestListModel(const TestListModel& rhs) : impl_(new Implementation(*this, rhs.impl_->shortList_))
{
}
