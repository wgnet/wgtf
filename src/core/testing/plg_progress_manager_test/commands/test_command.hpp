...
{
public:
	TestCommandBase(uint8_t processCounter = 0) : processCounter_(processCounter)
	{
	}
...
/// Test command class that will emulate 3 second wait and log the process to output window each second when the command
/// is executed
class TestCommand1 : public TestCommandBase
{
public:
	TestCommand1(uint8_t processCounter = 3) : TestCommandBase(processCounter)
	{
	}
...
/// is executed
class TestCommand2 : public TestCommandBase
{
public:
	TestCommand2(uint8_t processCounter = 5) : TestCommandBase(processCounter)
	{
	}
