...
static PyObject* py_create(PyObject* self, PyObject* args, PyObject* kw)
{
	if (g_module == nullptr)
	{
		PyErr_Format(PyExc_Exception, "Module is not loaded.");
		return nullptr;
	}
	...
	static char* keywords[] = { "objectType", nullptr };
	...
	if (objectType[0] == '\0')
	{
		PyErr_Format(PyExc_TypeError, "Can not create a definition from an empty string.");
		return nullptr;
	}

	auto pDefinitionManager = g_module->context_.queryInterface<IDefinitionManager>();
	if (pDefinitionManager == nullptr)
	{
		PyErr_Format(PyExc_Exception, "Could not get definition manager.");
		return nullptr;
	}

	const IClassDefinition* pDefinition = pDefinitionManager->getDefinition(objectType);
	if (pDefinition == nullptr)
	{
		PyErr_Format(PyExc_TypeError, "Could not find a definition for \"%s\"\n", objectType);
		return nullptr;
	}
 ...
 *	@return None.
 */
PyObject* parseArguments(PyObject* self, PyObject* args, PyObject* kw)
{
	PyObject* object = nullptr;

	static char* keywords[] = { "object", nullptr };
	...
	if (object == nullptr)
	{
		PyErr_Format(PyExc_TypeError, "Cannot test null object.");
		return nullptr;
	}
...
void noneConversionTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
void numericConversionTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
void stringConversionTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
void childConversionTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
void listConversionTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
void tupleConversionTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
void dictConversionTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
void methodConversionTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
void newPropertyTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
void pathTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
void compareTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_);
 ...
 *	@return None.
 */
static PyObject* commonConversionTest(ReflectedPython::DefinedInstance& instance)
{
	if (g_module == nullptr)
	{
		PyErr_Format(PyExc_Exception, "Module is not loaded.");
	...
	// instance
	{
		...
		const ClassDefinition* pGenericClassDefinition = dynamic_cast<const ClassDefinition*>(&genericDefinition);
		if (pGenericClassDefinition == nullptr)
		{
			PyErr_Format(PyExc_TypeError, "Failed to get correct definition.");
			return nullptr;
		}

		const IClassDefinitionDetails& details = pGenericClassDefinition->getDetails();
...
void noneConversionTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_)
{
	// Convert Python None -> C++ nullptr
	{
		...
		const bool setSuccess = instance.set<void*>("noneTest", noneType);

		CHECK(setSuccess);

		void* noneResult;
		const bool getSuccess = instance.get<void*>("noneTest", noneResult);
...
{
	{
		...
		const bool setSuccess = instance.set<std::string>("stringTest", stringExpected);

		CHECK(setSuccess);

		std::string stringResult;
		const bool getSuccess = instance.get<std::string>("stringTest", stringResult);
	...
	}
	{
		...
		const bool setSuccess = instance.set<std::wstring>("unicodeTest", unicodeExpected);
		CHECK(setSuccess);

		std::wstring unicodeResult = L"Fail";
		const bool getSuccess = instance.get<std::wstring>("unicodeTest", unicodeResult);
...
{
	{
		...
		const bool setSuccess = instance.set<std::string>("childTest.stringTest", childPropertyExpected);

		CHECK(setSuccess);

		std::string childPropertyResult;
		const bool getSuccess = instance.get<std::string>("childTest.stringTest", childPropertyResult);
...
void resetList(ReflectedPython::DefinedInstance& instance, const size_t size, const char* m_name, TestResult& result_)
{
	...
	const bool resetSuccess = instance.set<Collection>("listTest", listTest);

	CHECK(resetSuccess);
}

void checkList(const Collection& listResult, size_t expectedSize, const char* m_name, TestResult& result_)
{
	int i = 0;
...
{
	{
		...
		const bool getSuccess = instance.get<Collection>("listTest", listResult);
	...
	}
	{
		...
		const bool setSuccess = instance.set<int>("listTest[0]", listExpected);

		CHECK(setSuccess);

		int listResult = 0;
		const bool getSuccess = instance.get<int>("listTest[0]", listResult);
	...
	}
	{
		...
		const bool setSuccess = instance.set<int>("listTest[1]", listExpected);

		CHECK(setSuccess);

		int listResult = 0;
		const bool getSuccess = instance.get<int>("listTest[1]", listResult);
	...
	}
	{
		...
		// Out-of-range
		const int listExpected = 11;
		const bool setSuccess = instance.set<int>("listTest[999]", listExpected);

		CHECK(!setSuccess);

		int listResult = 0;
		const bool getSuccess = instance.get<int>("listTest[999]", listResult);
	...
	}
	{
		...
		{
			int listResult = 0;
			const bool getSuccess = instance.get<int>("listTest[4]", listResult);
	...
	}
	{
		...
		const int listExpected = 11;
		const bool setSuccess = instance.set<int>("listTest[-100]", listExpected);

		CHECK(!setSuccess);

		int listResult = 0;
		const bool getSuccess = instance.get<int>("listTest[-100]", listResult);
	...
	}
	{
		...
		const int listExpected = 10;
		const bool setSuccess = instance.set<int>("listTest[0][1]", listExpected);

		CHECK(setSuccess);

		int listResult = 0;
		const bool getSuccess = instance.get<int>("listTest[0][1]", listResult);
...
void resetTuple(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_)
{
	// Reset list in case another test above modified it
	...
	const bool resetSuccess = instance.set<Collection>("tupleTest", tupleTest);

	CHECK(resetSuccess);
}

void checkTuple(const Collection& tupleResult, size_t expectedSize, const char* m_name, TestResult& result_)
{
	int i = 0;
...
{
	{
		...
		const bool getSuccess = instance.get<Collection>("tupleTest", tupleResult);
	...
	}
	{
		...
		const bool setSuccess = instance.set<int>("tupleTest[0]", tupleExpected);

		CHECK(setSuccess);

		int tupleResult = 0;
		const bool getSuccess = instance.get<int>("tupleTest[0]", tupleResult);
	...
	}
	{
		...
		const bool setSuccess = instance.set<int>("tupleTest[1]", tupleExpected);

		CHECK(setSuccess);

		int tupleResult = 0;
		const bool getSuccess = instance.get<int>("tupleTest[1]", tupleResult);
	...
	}
	{
		...
		// Out-of-range
		const int tupleExpected = 11;
		const bool setSuccess = instance.set<int>("tupleTest[999]", tupleExpected);

		CHECK(!setSuccess);

		int tupleResult = 0;
		const bool getSuccess = instance.get<int>("tupleTest[999]", tupleResult);
	...
	}
	{
		...
		{
			int tupleResult = 0;
			const bool getSuccess = instance.get<int>("tupleTest[4]", tupleResult);
	...
	}
	{
		...
		const int tupleExpected = 11;
		const bool setSuccess = instance.set<int>("tupleTest[-100]", tupleExpected);

		CHECK(!setSuccess);

		int tupleResult = 0;
		const bool getSuccess = instance.get<int>("tupleTest[-100]", tupleResult);
	...
	}
	{
		...
		const int tupleExpected = 10;
		const bool setSuccess = instance.set<int>("listTest[0][1]", tupleExpected);

		CHECK(setSuccess);

		int tupleResult = 0;
		const bool getSuccess = instance.get<int>("listTest[0][1]", tupleResult);
...
void resetDict(ReflectedPython::DefinedInstance& instance, size_t size, const char* m_name, TestResult& result_)
{
	...
	const bool resetSuccess = instance.set<Collection>("dictTest", dictTest);

	CHECK(resetSuccess);
}

void checkDict(const Collection& dictResult, size_t expectedSize, const char* m_name, TestResult& result_)
{
	const size_t maxDigits = 10;
	char buffer[maxDigits];
...
{
	{
		...
		const bool getSuccess = instance.get<Collection>("dictTest", dictResult);
	...
	}
	{
		...
		int i = 0;
		std::generate(expectedValues.begin(), expectedValues.end(), [&i]() -> int { return i++; });
		for (auto itr = dictResult.cbegin(); itr != dictResult.cend(); ++itr)
		{
			...
			const auto foundItr = std::find(expectedValues.cbegin(), expectedValues.cend(), index);
	...
	}
	{
		...
		// Dict in dict
		{
			...
			container2[3] = 3;
			Collection tupleTest(container2);
			const bool resetSuccess = instance.set<Collection>("dictTest", tupleTest);

			CHECK(resetSuccess);
		}

		const int dictExpected = 10;
		const bool setSuccess = instance.set<int>("dictTest[0][1]", dictExpected);

		CHECK(setSuccess);

		int dictResult = 0;
		const bool getSuccess = instance.get<int>("dictTest[0][1]", dictResult);
...
{
	{
		// @see Py_None
		void* noneType = nullptr;
		const bool setSuccess = instance.set<void*>("newPropertyTest", noneType);

		CHECK(setSuccess);

		void* noneResult;
		const bool getSuccess = instance.get<void*>("newPropertyTest", noneResult);
 ...
 */
void getCollectionPath1(const ReflectedPython::DefinedInstance& instance, Collection& outCollection,
                        const char* collectionName, const char* m_name, TestResult& result_)
{
	ObjectHandle childHandle;
	const bool getChildSuccess = instance.get<ObjectHandle>("childTest", childHandle);
	...
	const bool getCollectionSuccess = child.get<Collection>(collectionName, outCollection);
 ...
 */
void getCollectionPath2(const ReflectedPython::DefinedInstance& instance, Collection& outCollection,
                        const char* collectionName, const char* m_name, TestResult& result_)
{
	std::string pathName = "childTest";
	pathName += IClassDefinition::DOT_OPERATOR;
	pathName += collectionName;
	const bool getCollectionSuccess = instance.get<Collection>(pathName.c_str(), outCollection);
 ...
 */
void checkSequencePaths(const ReflectedPython::DefinedInstance& instance, const Collection& collection,
                        const char* collectionName, const char* m_name, TestResult& result_)
{
	auto expectedKey = 0;
	for (auto itr = collection.cbegin(); itr != collection.cend(); ++itr, ++expectedKey)
	{
		int key = -1;
 ...
 */
void checkMappingPaths(const ReflectedPython::DefinedInstance& instance, const Collection& collection,
                       const char* collectionName, const char* m_name, TestResult& result_)
{
	for (auto itr = collection.cbegin(); itr != collection.cend(); ++itr)
	{
		...
		const std::string expectedKeyFullPath = keyInstance.pythonObject().str(PyScript::ScriptErrorPrint()).c_str();
...
void compareTest(ReflectedPython::DefinedInstance& instance, const char* m_name, TestResult& result_)
{
	...
	{
		ObjectHandle result;
		const bool getSuccess = instance.get<ObjectHandle>("badComparison", result);
 ...
 *	@return None.
 */
static PyObject* py_oldStyleConversionTest(PyObject* self, PyObject* args, PyObject* kw)
{
	if (g_module == nullptr)
	{
		PyErr_Format(PyExc_Exception, "Module is not loaded.");
	...
	auto checkObjectType = [&](const char* attribute, const char* expectedTypeBase) {
		...
		const auto& details = static_cast<const ReflectedPython::DefinitionDetails&>(definition->getDetails());
		std::string expectedType = expectedTypeBase;
		expectedType += " at ";
		expectedType += std::to_string(details.object().id().asUnsignedLongLong(PyScript::ScriptErrorRetain()));
 ...
 *	@return None.
 */
static PyObject* py_newStyleConversionTest(PyObject* self, PyObject* args, PyObject* kw)
{
	if (g_module == nullptr)
	{
		PyErr_Format(PyExc_Exception, "Module is not loaded.");
...

	{
		...
		const bool setSuccess = instance.set<std::string>("readOnlyPropertyTest1", stringTest);

		CHECK(!setSuccess);

		const std::string expectedString = "Read-only Property";
		std::string stringResult;
		const bool getSuccess = instance.get<std::string>("readOnlyPropertyTest1", stringResult);
...

	{
		// @see property() builtin, @property decorator
		const std::string stringTest = "String was set";
		const bool setSuccess = instance.set<std::string>("readOnlyPropertyTest2", stringTest);

		CHECK(!setSuccess);

		const std::string expectedString = "Read-only Property";
		std::string stringResult;
		const bool getSuccess = instance.get<std::string>("readOnlyPropertyTest2", stringResult);
...

	{
		...
		const std::string stringExpected = "String was set";
		const bool setSuccess = instance.set<std::string>("descriptorTest", stringExpected);

		CHECK(setSuccess);

		std::string stringResult;
		const bool getSuccess = instance.get<std::string>("descriptorTest", stringResult);
...
ReflectionTestModule::ReflectionTestModule(IComponentContext& context, const char* testName, TestResult& result)
    : context_(context), testName_(testName), result_(result)
{
	g_module = this;
	...
	if (Py_IsInitialized())
	{
		static PyMethodDef s_methods[] = {
			{ "create", reinterpret_cast<PyCFunction>(&py_create), METH_VARARGS | METH_KEYWORDS,
			  "Create C++ reflected object and return it to Python" },
			{ "oldStyleConversionTest", reinterpret_cast<PyCFunction>(&py_oldStyleConversionTest),
			  METH_VARARGS | METH_KEYWORDS, "Inspect an old-style Python class using the reflection system" },
			{ "newStyleConversionTest", reinterpret_cast<PyCFunction>(&py_newStyleConversionTest),
			  METH_VARARGS | METH_KEYWORDS, "Inspect a new-style Python class using the reflection system" },
