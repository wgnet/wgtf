...
struct RandomValueGenerator<std::shared_ptr<T>>
{
	...
	{
		return std::shared_ptr<T>(RandomValueGenerator<T*>::value(defManager));
	}
};

template <typename T, typename Collection>
void populateData(const IDefinitionManager& defManager, Collection& collection, size_t itemsToGenerate = 10)
{
	for (auto i = 0u; i < itemsToGenerate; ++i)
	{
		collection.push_back(RandomValueGenerator<T>::value(defManager));
	}
}
};
} // end namespace wgt
